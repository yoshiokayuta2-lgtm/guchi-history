(() => {
  ...
  function joinPdfTextItems(items){
    ...
    for(const item of items){
      ...
      const height=Math.max(Math.abs(Number(tr[3] || 0)), Math.abs(Number(item.height || 0)), 1);
      let row=rows.find(r=>Math.abs(r.y-y) <= Math.max(2.5, height*0.28));
    ...
    return rows.map(row=>{
      ...
      let out="";
      for(const part of row.items){
        const prev=out.slice(-1);
        const next=part.text.charAt(0);
        const latinBoundary=/[A-Za-z0-9]/.test(prev) && /[A-Za-z0-9]/.test(next);
        out += (out && latinBoundary ? " " : "") + part.text;
      }
      return normalizePdfLineText(out);
    }).filter(Boolean);
  }

  async function extractWorksheetPdfText(file){
    const pdfjs=window.pdfjsLib;
    if(!pdfjs || typeof pdfjs.getDocument !== "function"){
      throw new Error("PDF_TEXT_LIBRARY_UNAVAILABLE");
    }
    ...
    for(let pageNo=1; pageNo<=pdf.numPages; pageNo++){
      ...
      const lines=joinPdfTextItems(content.items || []);
      pages.push({pageNo,lines,text:lines.join("\n")});
    }
    const lines=pages.flatMap(p=>p.lines);
    return {pages,lines,text:pages.map(p=>p.text).join("\n\n")};
  }

  function scorePromptLine(line){
    let score=0;
    if(/(?:説明|述べ|論じ|答え|記述|書き|示し|明らかに).{0,12}(?:なさい|せよ|よ。|よ$)/.test(line)) score+=6;
    if(/(?:何|なぜ|どのよう|いかなる|どんな).{0,40}(?:か[。？?]?|説明)/.test(line)) score+=4;
    if(/\d{1,3}\s*字(?:以内|程度|以上|以下)?/.test(line)) score+=4;
    if(/^(?:問|設問|問題|\(?\d+\)?|[①-⑳])/.test(line)) score+=2;
    if(line.length>=20) score+=1;
    if(line.length>240) score-=2;
    return score;
  }

  function detectPromptFromPdfLines(lines){
    const cleaned=lines.map(normalizePdfLineText).filter(Boolean);
    let best={score:-1,text:"",index:-1};
    for(let i=0;i<cleaned.length;i++){
      const variants=[cleaned[i]];
      if(i+1<cleaned.length) variants.push(`${cleaned[i]}${cleaned[i+1]}`);
      if(i+2<cleaned.length) variants.push(`${cleaned[i]}${cleaned[i+1]}${cleaned[i+2]}`);
      for(const text of variants){
        const score=scorePromptLine(text);
        if(score>best.score && text.length>=8) best={score,text,index:i};
      }
    }
    if(best.score>=4) return best.text;
    const fallback=cleaned.find(x=>x.length>=25 && !/^(?:氏名|名前|クラス|組|番号|日付|解答|メモ|ポイント)/.test(x));
    return fallback || "";
  }

  function detectWorksheetSource(lines){
    const explicit=lines.find(x=>/^(?:出典|資料|引用)[:：]/.test(x));
    if(explicit) return explicit.replace(/^(?:出典|資料|引用)[:：]\s*/,"");
    const uni=lines.find(x=>/(?:19|20)\d{2}.{0,18}(?:大学|大入試)|(?:大学|大入試).{0,18}(?:19|20)\d{2}/.test(x));
    return uni || "";
  }

  function detectWorksheetTitle(lines,fileName,subject){
    const candidates=lines.filter(x=>x.length>=3 && x.length<=60 && !/^(?:氏名|名前|クラス|組|番号|日付|問\d*|設問|問題\d*)[:：]?/.test(x));
    const preferred=candidates.find(x=>/(?:日本史|世界史).{0,12}(?:論述|記述|演習|課題|プリント)|(?:論述|記述|演習|課題).{0,12}(?:日本史|世界史)/.test(x));
    if(preferred) return preferred;
    const heading=candidates.find(x=>/(?:論述|記述|演習|課題|第\d+回|第\d+週)/.test(x));
    if(heading) return heading;
    const stem=String(fileName || "").replace(/\.pdf$/i,"").trim();
    return stem || `${subject === "world" ? "世界史" : "日本史"} 論述課題`;
  }

  function parseExtractedWorksheetText(extracted,file){
    const lines=extracted.lines.map(normalizePdfLineText).filter(Boolean);
    const text=lines.join("\n");
    if(text.replace(/\s/g,"").length < 20){
      const err=new Error("PDF_IMAGE_ONLY");
      err.code="PDF_IMAGE_ONLY";
      throw err;
    }
    const worldCount=(text.match(/世界史/g)||[]).length + (text.match(/(?:ヨーロッパ|中国史|イスラーム|近代世界|古代世界)/g)||[]).length;
    const japaneseCount=(text.match(/日本史/g)||[]).length + (text.match(/(?:日本|幕府|天皇|朝廷|明治|江戸)/g)||[]).length;
    const subject=worldCount>japaneseCount ? "world" : "japanese";
    const prompt=detectPromptFromPdfLines(lines);
    const promptIndex=prompt ? text.indexOf(prompt) : -1;
    const nearby=promptIndex>=0 ? text.slice(Math.max(0,promptIndex-160),promptIndex+prompt.length+160) : text;
    const limitMatch=nearby.match(/(\d{1,3})\s*字(?:以内|程度|以上|以下)?/) || text.match(/(\d{1,3})\s*字(?:以内|程度|以上|以下)?/);
    const limit=limitMatch ? Math.max(10,Math.min(400,Number(limitMatch[1]))) : 50;
    const source=detectWorksheetSource(lines);
    const title=detectWorksheetTitle(lines,file.name,subject);
    return {
      demo:false,
      localPdfText:true,
      subject,
      ...
      theme:"",
      goal:"",
      method:"設問条件を分解する → 必須要素を置く → 字数内で1文にまとめる",
      source:source || file.name,
      prompt:prompt || lines.slice(0,Math.min(lines.length,8)).join("\n"),
      limit,
      beforeFields:["設問は何を聞いているか","時期・地域・人物"],
      requiredElements:["","",""],
      answerFlow:"",
      teacherModel:"",
      extractedText:text,
      summary:`PDFの文字データを直接読み取りました（${extracted.pages.length}ページ）。問題文・字数・出典などを候補入力しています。テーマ・到達目標・採点要素は先生が確認して補ってください。`
    };
  }
  ...
  async function parseWorksheetPdf(pdfDataUrl, file){
    ...
    const extracted=await extractWorksheetPdfText(file);
    return parseExtractedWorksheetText(extracted,file);
  }
          ...
          <div class="login-box card pad">
            ...
            <div class="grid two">
              ...
              <div class="role-card card" style="box-shadow:none">
                ...
                `}
                <div class="field" style="text-align:left"><label>ログインコード</label><input id="studentLoginCode" type="text" inputmode="numeric" autocomplete="one-time-code" maxlength="8" placeholder="先生から伝えられたコード"></div>
                <button class="btn" data-login-role="student">生徒として入る</button>
              </div>
            </div>
            <div class="footer">GUCHIの歴史添削隊 v6.3 / Japanese & World History Writing</div>
          </div>
        </div>
      </div>`;
        ...
        <div class="student-register-box">
          ...
          <form id="studentRegisterForm">
            <div class="grid two">
              ...
              <div class="field"><label>ログインコード</label><div class="input-with-button"><input id="studentLoginCodeNew" name="loginCode" type="text" inputmode="numeric" maxlength="8" placeholder="空欄なら4桁を自動発行"><button class="btn secondary" type="button" data-action="generate-student-code">自動発行</button></div></div>
            </div>
            <div class="field">
              <label>受講科目</label>
      ...
      <div class="card pad">
        ...
        <div class="pdf-import-panel">
          <div class="capture-step" style="margin-top:0">
            ...
            <div><h2>いつものプリントPDFをアップ</h2><p class="small muted">Wordなどから作成したPDFなら、文字データを直接読み取り、タイトル・設問・字数・出典を下の課題フォームへ振り分けます。</p></div>
          </div>
          ...
          <div class="ocr-actions" style="margin-top:12px">
            <button type="button" id="parseWorksheetButton" class="btn" data-action="parse-worksheet" disabled>${CONFIG.worksheetParseEndpoint ? "AIで課題化" : "PDFから読み取る"}</button>
        ...
        <div class="capture-step">
          ...
          <div><h2>読み取り結果を先生が微調整</h2><p class="small muted">問題文・字数・出典などを確認し、テーマや採点基準を必要に応じて補ってから公開します。</p></div>
        </div>
        <form id="assignmentForm">
          ...
          <h3>先生用：必ず入れる要素</h3>
          <p class="small muted">ここはPDFの文字抽出だけでは判断しにくいため、先生が添削基準として入力します。AI解析API接続後は候補生成にも対応できます。</p>
  ...
  function bindTeacher(){
    ...
    const fillAssignmentForm = (data) => {
      ...
      if(summary){ summary.style.display="block"; summary.textContent=(data.localPdfText ? "PDF文字読み取り完了：" : (data.demo ? "デモ解析：" : "AI解析完了：")) + (data.summary || "PDFの内容を課題フォームへ反映しました。先生が確認してから公開してください。"); }
    };
    const runWorksheetParse = async () => {
      ...
      if(worksheetParseStatus) worksheetParseStatus.textContent=CONFIG.worksheetParseEndpoint ? "PDFをAI解析しています…" : "PDF内の文字データを読み取っています…";
      try{
        const data = await parseWorksheetPdf(pendingWorksheetPdfDataUrl, file);
        fillAssignmentForm(data);
        if(worksheetParseStatus) worksheetParseStatus.textContent = data.localPdfText ? "文字読み取り完了。下の自動入力内容を確認・修正してください。" : (data.demo ? "デモ解析完了。下の自動入力内容を確認・修正してください。" : "解析完了。下の自動入力内容を確認・修正してください。");
        form?.scrollIntoView({behavior:"smooth",block:"start"});
      }catch(err){
        console.error(err);
        if(err?.code === "PDF_IMAGE_ONLY" || err?.message === "PDF_IMAGE_ONLY"){
          if(worksheetParseStatus) worksheetParseStatus.textContent="このPDFは文字データをほとんど含まない画像形式です。現状は手入力してください。AI解析API接続後は画像PDFにも対応できます。";
          alert("このPDFは画像形式のため、文字データを直接読み取れません。WordからPDF化したファイルなら読み取りできます。");
        }else if(err?.message === "PDF_TEXT_LIBRARY_UNAVAILABLE"){
          if(worksheetParseStatus) worksheetParseStatus.textContent="PDF文字読み取りライブラリを読み込めませんでした。通信状況を確認して再読み込みしてください。";
          alert("PDF文字読み取り機能の読み込みに失敗しました。ページを再読み込みしてください。");
        }else{
          if(worksheetParseStatus) worksheetParseStatus.textContent="解析に失敗しました。PDFを選び直すか、手入力してください。";
          alert(CONFIG.worksheetParseEndpoint ? "PDFのAI解析に失敗しました。" : "PDFの文字読み取りに失敗しました。");
        }
      }finally{
        if(parseWorksheetButton){ parseWorksheetButton.disabled=false; parseWorksheetButton.textContent=`${CONFIG.worksheetParseEndpoint ? "AIで課題化" : "PDFから読み取る"}`; }
      }
    };
    if(worksheetPdfInput) worksheetPdfInput.addEventListener("change", async () => {
      ...
      if(worksheetParseStatus) worksheetParseStatus.textContent=pendingWorksheetPdfStored ? (HAS_SUPABASE ? "PDFを選択しました。公開時にSupabase Storageへ保存します。" : "PDFを選択しました。文字データを読み取れます。") : "PDFは解析できますが、デモ保存容量の都合で原本PDFは課題に残しません。本番ではStorageに保存します。";
