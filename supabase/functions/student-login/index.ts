...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!secretKey) return json({ error: "Server secret is not configured." }, 500);
    ...
    if (userData.user.is_anonymous !== true) {
      return json({ error: "生徒ログインには匿名セッションが必要です。" }, 403);
    }

    const body = await req.json().catch(() => ({}));
    const name = String(body?.name || "").trim();
    const code = String(body?.code || "").trim();
    if (!name || !code) return json({ error: "名前とログインコードを入力してください。" }, 400);
    ...
    if (!candidates?.length) return json({ error: "名前またはログインコードが違います。" }, 401);
    ...
    if (!matched) return json({ error: "名前またはログインコードが違います。" }, 401);

    const { error: deviceError } = await admin
      .from("student_devices")
      .upsert(
        { student_id: matched.id, auth_user_id: userData.user.id },
