(() => {
  ...
  async function uploadWorksheetPdfIfNeeded(a){
    ...
    // Supabase Storage object keys follow AWS-style key naming rules.
    // Keep the original Japanese filename in worksheet_pdf_name, but use a stable
    // ASCII-only object key so Japanese / spaces / punctuation never cause InvalidKey.
    const path = `${a.id}/worksheet.pdf`;
  ...
  async function parseWorksheetPdf(pdfDataUrl, file){
    ...
    // API未接続時は、過去の日本史デモ課題を流用しない。
    // ファイル名から科目だけ推定し、毎回そのPDF専用の空フォームを作る。
    const stem = file.name.replace(/\.pdf$/i, "");
    const inferredSubject = /世界史|world/i.test(file.name)
      ? "world"
      : /日本史|japanese/i.test(file.name)
        ? "japanese"
        : "japanese";
    ...
    return {
      ...
      answerFlow:"",
      teacherModel:"",
      summary:`API未接続のためPDF本文はまだ解析していません。${inferredSubject === "world" ? "世界史" : "日本史"}としてファイル名だけ反映しました。問題文などは先生が入力してください。`
    };
  }
      ...
      <div class="app-shell">
        <div class="container">
          <div class="login-box card pad">
            ...
            <div class="footer">GUCHIの歴史添削隊 v6.2.3 / Japanese & World History Writing</div>
          </div>
        </div>
      </div>`;
  ...
  function bindTeacher(){
    ...
    const runWorksheetParse = async () => {
      ...
      try{
        ...
        if(worksheetParseStatus) worksheetParseStatus.textContent = data.demo ? "API未接続：科目とファイル名だけ反映しました。問題文などを入力してください。" : "解析完了。下の自動入力内容を確認・修正してください。";
        ...
        if(parseWorksheetButton){ parseWorksheetButton.disabled=false; parseWorksheetButton.textContent=`AIで課題化${CONFIG.worksheetParseEndpoint ? "" : "（API未接続）"}`; }
      }
    };
