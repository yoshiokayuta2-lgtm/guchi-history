...
- 生徒登録：先生画面から可能
- 日本史 / 世界史：科目を明示して課題管理
- PDF課題化：Word等から作成した文字データ入りPDFは、APIなしでブラウザ内解析して問題文・字数・出典などを自動入力
- 画像PDF：現状は文字抽出不可。AI解析API接続後に対応
- 答案写真提出：実装済み
...
フッターに `v6.3` と表示されれば最新ファイルがGitHub Pagesに反映されています。
...
- 生徒ログインの「ログインコード」欄を、名前・メール・パスワード欄と同じ入力デザインに統一
- 型指定のない入力欄も共通CSSが適用されるよう修正
- ログインコード入力に `type="text"` / `inputmode="numeric"` を明示


## v6.3 PDF文字読み取り

- OpenAI API未契約でも、Word等から作成した文字データ入りPDFをPDF.jsでブラウザ内解析
- 科目・課題タイトル・問題文・字数制限・出典を候補として自動入力
- テーマ・到達目標・必須要素・模範解答は文字抽出だけでは確定できないため先生が補う
- 画像PDFだった場合は「画像形式」と明示し、AI解析API接続後に対応
- API接続後は `worksheetParseEndpoint` を設定すればAI解析を優先
