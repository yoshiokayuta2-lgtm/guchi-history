...
- 日本語・空白を含むPDFファイル名でSupabase Storageが `Invalid key` を返す問題を修正。
- Storage上のオブジェクト名は `<課題ID>/worksheet.pdf` に固定。
- 元の日本語ファイル名は `worksheet_pdf_name` にそのまま保存して画面表示に使用。
- 日本史・世界史どちらのPDFでも同じ方式で保存。


## v6.2.4 科目混在バグ修正

- 世界史PDFでもファイル名に「論述」が含まれると、日本史1回目の固定デモ内容が入る不具合を修正。
- API未接続時は過去課題を流用せず、ファイル名から日本史/世界史だけ推定して新規フォームを初期化。
- API未接続であることを画面上で明示し、PDF本文を読んだように見える表示を廃止。
