...
- Supabase Storage の `Invalid key` 対策。日本語PDF名をStorage keyに直接使わないよう変更。
- 課題PDFのStorage pathを `<assignment-id>/worksheet.pdf` に固定。
- 元ファイル名はDB/UI側で保持。
- キャッシュ回避のため app/styles/config を v623 固定名に更新。


## v6.2.4

- 世界史の論述PDFが日本史1回目の固定デモ課題として表示される不具合を修正。
- 原因: API未接続時のデモ判定正規表現に「論述」が含まれており、世界史論述演習も日本史デモ分岐へ入っていた。
- API未接続時は科目/ファイル名のみ反映し、以前の課題内容を再利用しないよう変更。
