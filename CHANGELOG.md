...
- WordなどからPDF化した「文字データ入りPDF」を、OpenAI APIなしでブラウザ内読み取りできるように変更
- PDF.jsでページ内テキストを抽出し、科目・課題タイトル・問題文・字数制限・出典を候補入力
- テーマ・到達目標・必須要素など、文字抽出だけで確定しにくい欄は先生が補う設計
- 画像だけのPDFを検出した場合は、AI解析API接続後に対応できる旨を明示
- 「AIで課題化（デモ）」表示を廃止し、API未接続時は「PDFから読み取る」に変更
